...
def create_naive_env(params, random_seed=None):
...
def create_kb_env(params, random_seed=None):
    if random_seed is None:
        random_seed = params.random_seed
...
def run_experiment(params):
    ...
    trial_results = []
    for start_episode in (0, params.num_episodes):
        trial_results.append(train_and_evaluate(
            create_env(params, start_episode + params.random_seed),
            agent,
...
PSPACE = PermutationSpace(
    ['random_seed', 'max_internal_actions', 'agent_type'],
    random_seed=[
    ...
    (lambda agent_type: agent_type == 'naive'),
    (lambda max_internal_actions: max_internal_actions == 0),
).filter_if(
    # set internal actions appropriately for the album_date experiment
    (lambda agent_type: agent_type == 'kb'),
    (lambda max_internal_actions: max_internal_actions in (1, 4)),
)
