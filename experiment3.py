...
def create_naive_env(params, data_file=None, random_seed=None, whitelist=None):
...
def create_kb_env(params, data_file=None, random_seed=None, whitelist=None):
    if data_file is None:
        data_file = params.data_file
    if random_seed is None:
        random_seed = params.random_seed
...
def create_env(params, data_file=None, random_seed=None, whitelist=None):
    if params.agent_type == 'naive':
        return create_naive_env(params, data_file, random_seed, whitelist)
    elif params.agent_type == 'kb':
        return create_kb_env(params, data_file, random_seed, whitelist)
    raise ValueError(f'unknown agent type {params.agent_type}')
...
def run_experiment(params):
    ...
    whitelist = rng.sample(WHITELIST, params.num_albums)
    episodes = range(
        0,
        # end is num_episodes, plus however it goes over by
        (2 * params.num_episodes) + (params.eval_frequency // 2),
        params.eval_frequency,
    )
    trial_results = []
    trial_results.append(train_and_evaluate(
        create_env(params, data_file='album_artist', random_seed=params.random_seed, whitelist=whitelist),
        agent,
    ...
    trial_results.append(train_and_evaluate(
        create_env(params, data_file='album_date', random_seed=params.random_seed, whitelist=whitelist),
        agent,
        num_episodes=params.num_episodes,
        eval_frequency=params.eval_frequency,
        min_return=params.min_return,
    ...
    try:
        for episode, mean_return in zip(episodes, chain(*trial_results)):
...
PSPACE = PermutationSpace(
    ...
    data_file='album_date',
    results_folder='experiment3',
    num_transfers=[1],
    min_return=-100,
)
